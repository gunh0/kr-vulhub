...
print() 
```
5. 실행 결과로 출력된 요청 URL을 복사하여 웹 사이트에 입력
7. 템플릿 구문으로 입력된 내용이 실행되며 서버 프로세스의 id가 출력됨
8. PoC 코드를 조금 수정해 ls ../ 명령어가 실행되도록 하면
```python
from urllib import parse
...
print()
```
10. 명령어가 실행되며 상위 디렉토리가 나열되는 것을 확인 (다양한 명령어 실행이 가능한 것)
